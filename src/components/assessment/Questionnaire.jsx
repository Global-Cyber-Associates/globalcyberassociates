...
import { getActiveQuestionnaireConfig } from '../../config/questionnaireSelector';
...
export default function Questionnaire() {
    const [currentStep, setCurrentStep] = useState('firm-type');
    ...
    const handleNext = (data) => {
        ...
        if (nextIndex < STEPS.length - 1) {
            ...
            const finalResult = calculateScoreFromConfig(newAnswers, config);
...
function ToggleCard({ title, description, icon: Icon, checked, onChange }) {
    return (
        <div
            onClick={onChange}
            className={cn(
